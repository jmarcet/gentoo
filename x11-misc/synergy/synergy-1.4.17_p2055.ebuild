...
src_prepare() {
	epatch "${FILESDIR}/${PN}-1.4.16_p1969-pthread.patch"
	epatch "${FILESDIR}/${PN}-1.4.17_p2055-cryptopp.patch"

	mkdir -p "${S}/ext/gmock-1.6.0"
	cd "${S}/ext/gmock-1.6.0"
	unzip -qq -x ../gmock-1.6.0.zip
	mkdir -p "${S}/ext/gtest-1.6.0"
	cd "${S}/ext/gtest-1.6.0"
	unzip -qq -x ../gtest-1.6.0.zip
}
