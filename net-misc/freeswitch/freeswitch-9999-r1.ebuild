...
EGIT_BOOTSTRAP=""
EGIT_MASTER="2ab1605a8887adc62be1b75f6ef67af87ff080de"
inherit git-2
...
src_unpack() {
	...
	cd "${WORKDIR}/${P}"
	for i in `grep -l -r -e Wall -e Werror .`; do
		sed -e 's:\-Wall::g' -e 's:\-Werror::g' -i ${i}
	done
...
src_install() {
	...
	#
	find "${D}" \( -name "mod_*.la" -or -name "mod_*.a" -or -name "lib*.a" -or -name "ftmod_*.la" -or -name "ftmod_*.a" \) \
		-exec rm -f "{}" \; || die "Failed to cleanup .a and .la files"
