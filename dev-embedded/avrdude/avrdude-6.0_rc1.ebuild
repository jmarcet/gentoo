...
SRC_URI="mirror://nongnu/${PN}/${MY_P}.tar.gz
	doc? (
		mirror://nongnu/${PN}/${PN}-doc-${DOC_PV}.tar.gz
		mirror://nongnu/${PN}/${PN}-doc-${DOC_PV}.pdf
	)"
...
src_prepare() {
	...
	epatch "${FILESDIR}"/libftdi1_only.patch
}
...
src_install() {
	...
	if use doc ; then
		newdoc "${DISTDIR}/${PN}-doc-${DOC_PV}.pdf" avrdude.pdf
