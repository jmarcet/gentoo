...
IUSE="dbus debug designer doc examples kde multimedia opengl print qml sql svg webkit xmlpatterns"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
"
...
src_prepare() {
	...
	epatch "${FILESDIR}/0001-QtPrintSupport.patch"
...
src_configure() {
	configuration() {
		local myconf=(
			...
			RANLIB=
			STRIP=
			--qmake /usr/lib/qt5/bin/qmake --sip /usr/bin/sip
		)
		echo "${myconf[@]}"
		"${myconf[@]}" || die
