...
DESCRIPTION="Media Player for Linux with VAAPI support"
...
src_unpack() {
	...
	local ffrevision=$(grep 'ffrevision=' configure | cut -d'=' -f2)
	cd ffmpeg
	git checkout $ffrevision
	cd "${WORKDIR}"
...
src_prepare() {
	...
	REV=$(sed -n -e '/^dir$/{n;p;q;}' .svn/entries)
	printf "${REV}" > $svf
	printf "SVN-r${REV} (Gentoo)" > VERSION
	sh version.sh
