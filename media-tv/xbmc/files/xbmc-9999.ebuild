...
LICENSE="GPL-2"
SLOT="0"
IUSE="airplay airtunes alsa altivec avahi bluetooth bluray caps cec css debug +fishbmc gles goom java joystick midi mysql mythtv neon nfs +opengl profile +projectm pulseaudio rsxs rtmp +samba +sdl sse sse2 sftp udev upnp +usb vaapi vdpau webserver +X +xrandr"
REQUIRED_USE="
	mythtv? ( mysql )
	rsxs? ( X )
	X? ( sdl )
...
src_prepare() {
	...
	#epatch "${FILESDIR}"/${PN}-no-arm-flags.patch #400617
