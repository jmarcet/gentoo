...
case ${PV} in
9999)
	EGIT_REPO_URI="git://git.marcet.info/xbmc.git"
	EGIT_BRANCH="Gotham-13.2_rc1"
	inherit git-2
	KEYWORDS="~amd64 ~x86"
...
LICENSE="GPL-2"
SLOT="0"
IUSE="airplay airtunes alsa altivec avahi bluetooth bluray caps cec css debug +fishbmc gles goom java joystick midi mysql mythtv nfs +opengl profile +projectm pulseaudio +rsxs rtmp +samba +sdl sse sse2 sftp test udisks upnp upower +usb vaapi vdpau webserver +X +xrandr"
REQUIRED_USE="
	mythtv? ( mysql )
	rsxs? ( X )
	X? ( sdl )
...
COMMON_DEPEND="${PYTHON_DEPS}
	...
	media-sound/wavpack
	=media-video/ffmpeg-2.2_alpha1[encode]
	...
	vdpau? (
		...
		=media-video/ffmpeg-2.2_alpha1[vdpau]
	)
	X? (
		x11-apps/xdpyinfo
		x11-apps/mesa-progs
